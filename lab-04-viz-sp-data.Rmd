...
Each observation in this daataset represents a state, including DC.
...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents the data for one dennys resturaunt. The variables are: `r names(dennys)`.*** 

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?

***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents the data for one La Quinta. The variables are: `r names(laquinta)`.***
...
***La Quinta hotels are mostly located in the Americas, but are also in other parts of the world. Denny's are located in the US.*** 
...
***Using the state variable to determine if the location is the US by comparing the states in the database to a dataset of US states.***
...
***There are no Denny's outside of the US in our data.***
...
***It makes sense to me, because states like Delaware are smaller and Washington DC is also very small, especially when comparing to Texas or California.***
...
```
***The joke matches in some places, but also not every single La Quinta or Denny's is right next to each other***
...
```
***The joke matches a lot better in Texas than North Carolina, espeiclly in those masses of points where Denny's and La Quinta's are piled on top of each other.***
